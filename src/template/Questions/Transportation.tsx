...
const Transportation = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const transportationQuestions = questionnaire.TRANSPORTATION || [];

    console.log('transportationQuestions -->', transportationQuestions);


    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.TRANSPORT || Array(transportationQuestions.length).fill('')
    );
    const handleNext = async () => {
        const allAnswered = selectedAnswers.every(ans => ans !== '');
        if (!allAnswered) return;

        const formattedResponses = transportationQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {transportationQuestions.length > 0 && transportationQuestions.map((q, index) => (
