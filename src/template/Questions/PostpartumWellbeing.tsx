...
const PostpartumWellbeing = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const postpartumQuestions = questionnaire.POSTPARTUM_WELLBEING || [];

    console.log('postpartumQuestions -->', postpartumQuestions);

    // const [selectedAnswers, setSelectedAnswers] = useState<string[]>(Array(postpartumQuestions.length).fill(''));

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.POSTPARTUM || Array(postpartumQuestions.length).fill('')
    );
    const handleNext = async () => {
        const allAnswered = selectedAnswers.every(ans => ans !== '');
        if (!allAnswered) return;

        const formattedResponses = postpartumQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('post_partum.title')}</Text>

                {/* Dynamically render questions */}
                {postpartumQuestions.length > 0 && postpartumQuestions.map((q, index) => (
...
export default PostpartumWellbeing;
