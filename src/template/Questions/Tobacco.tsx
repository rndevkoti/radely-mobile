...
const Tobacco = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const tobaccoQuestions = questionnaire.TOBACCO_USE || [];

    console.log('tobaccoQuestions -->', tobaccoQuestions);

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.TOBACCO || Array(tobaccoQuestions.length).fill('')
    );
    ...
    const handleNext = async () => {
        ...
        const formattedResponses = tobaccoQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('tobacco.title')}</Text>

                {/* Dynamically render questions */}
                {tobaccoQuestions.length > 0 && tobaccoQuestions.map((q, index) => (
