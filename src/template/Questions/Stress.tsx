...
const Stress = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const stressQuestions = questionnaire.STRESS_MANAGEMENT || [];

    console.log('stressQuestions -->', stressQuestions);

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.STREES || Array(stressQuestions.length).fill('')
    );
    ...
    const handleNext = async () => {
        ...
        const formattedResponses = stressQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('stress.title')}</Text>

                {/* Dynamically render questions */}
                {stressQuestions.length > 0 && stressQuestions.map((q, index) => (
