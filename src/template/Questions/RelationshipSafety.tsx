...
const RelationshipSafety = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const relationshipsafetyQuestions = questionnaire.RELATIONSHIP_SAFETY || [];

    console.log('relationshipsafetyQuestions -->', relationshipsafetyQuestions);

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.RELATIONSHIP || Array(relationshipsafetyQuestions.length).fill('')
    );
    ...
    const handleNext = async () => {
        ...
        const formattedResponses = relationshipsafetyQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('relationship_safety')}</Text>

                {/* Dynamically render questions */}
                {relationshipsafetyQuestions.length > 0 && relationshipsafetyQuestions.map((q, index) => (
...
export default RelationshipSafety;
