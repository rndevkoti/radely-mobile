...
import { HEALTHCOMPLETESCREEN, HEALTHONBOARDINGSCREEN } from '../../appconstants';
import { windowHeight } from '../../utils/heightWidth';
import { submitHealthResponses } from '../../networking/submitAnswer';

const Utilities = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const utilitiesQuestions = questionnaire.UTILITIES || [];

    console.log('utilitiesQuestions -->', utilitiesQuestions);

    const onHandleLastContinue = () => {
        navigation.navigate(HEALTHCOMPLETESCREEN);
        setLastVisitedQuestionStep(0);
    }


    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.UTILITY || Array(utilitiesQuestions.length).fill('')
    );
    const handleNext = async () => {
        const allAnswered = selectedAnswers.every(ans => ans !== '');
        if (!allAnswered) return;

        const formattedResponses = utilitiesQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('utilities.title')}</Text>

                {/* Dynamically render questions */}
                {utilitiesQuestions.length > 0 && utilitiesQuestions.map((q, index) => (
