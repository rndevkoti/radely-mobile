...
const FinancialResources = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const financialQuestions = questionnaire.FINANCIAL_RESOURCES || [];
    const { answers, setAnswers } = useProfile();

    const { setHealthPayloads, healthPayloads } = useProfile();

    console.log('financialQuestions -->', financialQuestions);

    // const [selectedAnswers, setSelectedAnswers] = useState<string[]>(Array(financialQuestions.length).fill(''));

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.FINANCIAL || Array(financialQuestions.length).fill('')
    );
    const handleNext = async () => {
        const allAnswered = selectedAnswers.every(ans => ans !== '');
        if (!allAnswered) return;

        const formattedResponses = financialQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('financial_resources')}</Text>

                {/* Dynamically render questions */}
                {financialQuestions.length > 0 && financialQuestions.map((q, index) => (
