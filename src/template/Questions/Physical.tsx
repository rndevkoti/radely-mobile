...
const Physical = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const physicalactivityQuestions = questionnaire.PHYSICAL_ACTIVITY || [];

    console.log('physicalactivityQuestions -->', physicalactivityQuestions);

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.PHYSICAL || Array(physicalactivityQuestions.length).fill('')
    );
    ...
    const handleNext = async () => {
        ...
        const formattedResponses = physicalactivityQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('physical_activity.title')}</Text>

                {/* Dynamically render questions */}
                {physicalactivityQuestions.length > 0 && physicalactivityQuestions.map((q, index) => (
