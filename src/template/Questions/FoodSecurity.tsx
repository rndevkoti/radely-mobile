...
const FoodSecurity = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const foodsecurityQuestions = questionnaire.FOOD_SECURITY || [];

    console.log('foodsecurityQuestions -->', foodsecurityQuestions);


    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.FOOD || Array(foodsecurityQuestions.length).fill('')
    );
    const handleNext = async () => {
        const allAnswered = selectedAnswers.every(ans => ans !== '');
        if (!allAnswered) return;

        const formattedResponses = foodsecurityQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {foodsecurityQuestions.length > 0 && foodsecurityQuestions.map((q, index) => (
