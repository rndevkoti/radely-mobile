...
const HousingSecurity = ({ onContinue, step }: { onContinue: () => void; step: number }) => {
    ...
    const housingsecurityQuestions = questionnaire.HOUSING_SECURITY || [];

    console.log('housingsecurityQuestions -->', housingsecurityQuestions);

    // const [selectedAnswers, setSelectedAnswers] = useState<string[]>(Array(housingsecurityQuestions.length).fill(''));

    const [selectedAnswers, setSelectedAnswers] = useState<string[]>(
        answers.HOUSE || Array(housingsecurityQuestions.length).fill('')
    );
    ...
    const handleNext = async () => {
        ...
        const formattedResponses = housingsecurityQuestions.map((question, idx) => {
    ...
    return (
        <LinearGradient colors={theme.gradient} style={styles.container}>
            <View style={styles.scrollContainer}>
                ...
                {/* Title */}
                <Text style={styles.title}>{t('housing_security')}</Text>

                {/* Dynamically render questions */}
                {housingsecurityQuestions.length > 0 && housingsecurityQuestions.map((q, index) => (
